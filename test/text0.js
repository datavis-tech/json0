...
describe('text0', function() {
  describe('compose', () =>
    ...
    it('is sane', function() {
      ...
      assert.deepEqual(text0.compose([], [{i:'x', p:0}]), [{i:'x', p:0}]);
      return assert.deepEqual(text0.compose([{i:'y', p:100}], [{i:'x', p:0}]), [{i:'y', p:100}, {i:'x', p:0}]);
  })
);
  ...
  describe('transform', function() {
    it('is sane', function() {
      ...
      return assert.deepEqual([], text0.transform([], [{i:'y', p:100}, {i:'x', p:0}], 'right'));
    });
    ...
    it('inserts', function() {
      ...
      return assert.deepEqual([{i:'x', p:10}], text0.transform([{i:'x', p:10}], [{d:'a', p:10}], 'right'));
    });
    ...
    return it('deletes', function() {
      ...
      return assert.deepEqual([[{d:'abc', p:10}], [{d:'xy', p:10}]], text0.transformX([{d:'abc', p:10}], [{d:'xy', p:13}]));
  });
});
  ...
  describe('transformCursor', function() {
    ...
    it('works vs insert', function() {
      ...
      return assert.strictEqual(100, text0.transformCursor(100, [{i:'asdf', p:100}], 'left'));
    });
    ...
    return it('works vs delete', function() {
      ...
      return assert.strictEqual(101, text0.transformCursor(105, [{d:'asdf', p:100}]));
  });
});
  ...
  describe('normalize', function() {
    ...
    it('adds missing p:0', function() {
      ...
      return assert.deepEqual([{i:'abc', p:0}, {d:'abc', p:0}], text0.normalize([{i:'abc'}, {d:'abc'}]));
  });

    it('converts op to an array', function() {
      assert.deepEqual([{i:'abc', p:0}], text0.normalize({i:'abc', p:0}));
      return assert.deepEqual([{d:'abc', p:0}], text0.normalize({d:'abc', p:0}));
  });

    it('works with a really simple op', () => assert.deepEqual([{i:'abc', p:0}], text0.normalize({i:'abc'})));
    ...
    it('compress inserts', function() {
      ...
      return assert.deepEqual([{i:'abcxyz', p:10}], text0.normalize([{i:'abc', p:10}, {i:'xyz', p:13}]));
  });
    ...
    it('compress deletes', function() {
      ...
      return assert.deepEqual([{d:'abcxy', p:10}], text0.normalize([{d:'abc', p:10}, {d:'xy', p:10}]));
  });
  ...
  return describe('randomizer', () => it('passes', function() {
    this.timeout(4000);
    this.slow(4000);
