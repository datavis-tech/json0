...
const nativetype = require('../lib/json0');
...
const genTests = function(type) {
  ...
  describe('sanity', function() {
    describe('#create()', () => it('returns null', () => assert.deepEqual(type.create(), null))
     );

    describe('#compose()', function() {
      it('od,oi --> od+oi', function() {
        assert.deepEqual([{p:['foo'], od:1, oi:2}], type.compose([{p:['foo'],od:1}],[{p:['foo'],oi:2}]));
        return assert.deepEqual([{p:['foo'], od:1},{p:['bar'], oi:2}], type.compose([{p:['foo'],od:1}],[{p:['bar'],oi:2}]));
    });
      return it('merges od+oi, od+oi -> od+oi', () => assert.deepEqual([{p:['foo'], od:1, oi:2}], type.compose([{p:['foo'],od:1,oi:3}],[{p:['foo'],od:3,oi:2}])));
  });


    return describe('#transform()', () => it('returns sane values', function() {
      const t = function(op1, op2) {
      ...
      t([], []);
      t([{p:['foo'], oi:1}], []);
      return t([{p:['foo'], oi:1}], [{p:['bar'], oi:2}]);
  }) );
});
  ...
  describe('number', function() {
    it('Adds a number', function() {
      ...
      return assert.deepEqual([3], type.apply([1], [{p:[0], na:2}]));
  });

    it('compresses two adds together in compose', function() {
      assert.deepEqual([{p:['a', 'b'], na:3}], type.compose([{p:['a', 'b'], na:1}], [{p:['a', 'b'], na:2}]));
      return assert.deepEqual([{p:['a'], na:1}, {p:['b'], na:2}], type.compose([{p:['a'], na:1}], [{p:['b'], na:2}]));
  });
    ...
    return it('doesn\'t overwrite values when it merges na in append', function() {
      ...
      const rightOp = [{"p":[],"od":0,"oi":15},{"p":[],"na":4},{"p":[],"na":1},{"p":[],"na":1}];
      const leftOp = [{"p":[],"na":4},{"p":[],"na":-1}];
      const [right_, left_] = Array.from(transformX(type, rightOp, leftOp));
  ...
  describe('string', function() {
    describe('#apply()', () => it('works', function() {
      assert.deepEqual('abc', type.apply('a', [{p:[1], si:'bc'}]));
      assert.deepEqual('bc', type.apply('abc', [{p:[0], sd:'a'}]));
      return assert.deepEqual({x:'abc'}, type.apply({x:'a'}, [{p:['x', 1], si:'bc'}]));
  }) );

    return describe('#transform()', function() {
      it('splits deletes', () => assert.deepEqual(type.transform([{p:[0], sd:'ab'}], [{p:[1], si:'x'}], 'left'), [{p:[0], sd:'a'}, {p:[1], sd:'b'}]));

      it('cancels out other deletes', () => assert.deepEqual(type.transform([{p:['k', 5], sd:'a'}], [{p:['k', 5], sd:'a'}], 'left'), []));

      return it('does not throw errors with blank inserts', () => assert.deepEqual(type.transform([{p: ['k', 5], si:''}], [{p: ['k', 3], si: 'a'}], 'left'), []));
  });
});
  ...
  describe('string subtype', function() {
    describe('#apply()', () =>
      it('works', function() {
        ...
        return assert.deepEqual({x:'abc'}, type.apply({x:'a'}, [{p:['x'], t:'text0', o:[{p:1, i:'bc'}]}]));
    })
  );
    ...
    return describe('#transform()', function() {
      it('splits deletes', function() {
        ...
        return assert.deepEqual(type.transform(a, b, 'left'), [{p:[], t:'text0', o:[{p:0, d:'a'}, {p:1, d:'b'}]}]);
    });

      it('cancels out other deletes', () => assert.deepEqual(type.transform([{p:['k'], t:'text0', o:[{p:5, d:'a'}]}], [{p:['k'], t:'text0', o:[{p:5, d:'a'}]}], 'left'), []));

      return it('does not throw errors with blank inserts', () => assert.deepEqual(type.transform([{p:['k'], t:'text0', o:[{p:5, i:''}]}], [{p:['k'], t:'text0', o:[{p:3, i:'a'}]}], 'left'), []));
  });
});
  ...
  describe('subtype with non-array operation', () =>
    describe('#transform()', () =>
      it('works', function() {
        const a = [{p:[], t:'mock', o:'foo'}];
        const b = [{p:[], t:'mock', o:'bar'}];
        return assert.deepEqual(type.transform(a, b, 'left'), [{p:[], t:'mock', o:{mock:true}}]);
    })
  )
);
  ...
  describe('list', function() {
    describe('apply', function() {
      it('inserts', function() {
        ...
        return assert.deepEqual(['a', 'b', 'c'], type.apply(['a', 'b'], [{p:[2], li:'c'}]));
    });

      it('deletes', function() {
        assert.deepEqual(['b', 'c'], type.apply(['a', 'b', 'c'], [{p:[0], ld:'a'}]));
        assert.deepEqual(['a', 'c'], type.apply(['a', 'b', 'c'], [{p:[1], ld:'b'}]));
        return assert.deepEqual(['a', 'b'], type.apply(['a', 'b', 'c'], [{p:[2], ld:'c'}]));
    });

      it('replaces', () => assert.deepEqual(['a', 'y', 'b'], type.apply(['a', 'x', 'b'], [{p:[1], ld:'x', li:'y'}])));

      return it('moves', function() {
        assert.deepEqual(['a', 'b', 'c'], type.apply(['b', 'a', 'c'], [{p:[1], lm:0}]));
        return assert.deepEqual(['a', 'b', 'c'], type.apply(['b', 'a', 'c'], [{p:[0], lm:1}]));
    });
    ...
    describe('#transform()', function() {
      it('bumps paths when list elements are inserted or removed', function() {
        ...
        return assert.deepEqual([{p:[1],ld:2}], type.transform([{p:[0],ld:2}], [{p:[0],li:1}], 'right'));
      });
      ...
      it('converts ops on deleted elements to noops', function() {
        ...
        return assert.deepEqual([], type.transform([{p:[0],na:-3}], [{p:[0],ld:48}], 'left'));
      });
      ...
      it('converts ops on replaced elements to noops', function() {
        ...
        return assert.deepEqual([{p:[0], li:'hi'}], type.transform([{p:[0], li:'hi'}], [{p:[0], ld:'x', li:'y'}], 'left'));
      });

      it('changes deleted data to reflect edits', function() {
        assert.deepEqual([{p:[1], ld:'abc'}], type.transform([{p:[1], ld:'a'}], [{p:[1, 1], si:'bc'}], 'left'));
        return assert.deepEqual([{p:[1], ld:'abc'}], type.transform([{p:[1], ld:'a'}], [{p:[1], t:'text0', o:[{p:1, i:'bc'}]}], 'left'));
      });
      ...
      it('Puts the left op first if two inserts are simultaneous', function() {
        ...
        return assert.deepEqual([{p:[2], li:'b'}], type.transform([{p:[1], li:'b'}], [{p:[1], li:'a'}], 'right'));
      });
      ...
      return it('converts an attempt to re-delete a list element into a no-op', function() {
        ...
        return assert.deepEqual([], type.transform([{p:[1], ld:'x'}], [{p:[1], ld:'x'}], 'right'));
      });
    });
    ...
    describe('#compose()', function() {
      it('composes insert then delete into a no-op', function() {
        ...
        return assert.deepEqual([{p:[1],ld:null,li:'x'}], type.transform([{p:[0],ld:null,li:"x"}], [{p:[0],li:"The"}], 'right'));
      });
      ...
      it('doesn\'t change the original object', function() {
        ...
        return assert.deepEqual([{p:[0],ld:'abc',li:null}], a);
      });

      return it('composes together adjacent string ops', function() {
        assert.deepEqual([{p:[100], si:'hi'}], type.compose([{p:[100], si:'h'}], [{p:[101], si:'i'}]));
        return assert.deepEqual([{p:[], t:'text0', o:[{p:100, i:'hi'}]}], type.compose([{p:[], t:'text0', o:[{p:100, i:'h'}]}], [{p:[], t:'text0', o:[{p:101, i:'i'}]}]));
    });
  });
    ...
    it('moves ops on a moved element with the element', function() {
      ...
      return assert.deepEqual([{p:[2],li:'x'}], type.transform([{p:[2],li:'x'}], [{p:[0],lm:1}], 'left'));
    });
    ...
    it('moves target index on ld/li', function() {
      ...
      return assert.deepEqual([{p:[1],lm:1}], type.transform([{p:[0], lm: 0}], [{p:[0], li:28}], 'left'));
    });
    ...
    it('tiebreaks lm vs. ld/li', function() {
      ...
      return assert.deepEqual([{p:[1], lm:3}], type.transform([{p:[0], lm: 2}], [{p:[0], li:'x'}], 'right'));
    });

    it('replacement vs. deletion', () => assert.deepEqual([{p:[0],li:'y'}], type.transform([{p:[0],ld:'x',li:'y'}], [{p:[0],ld:'x'}], 'right')));

    it('replacement vs. insertion', () => assert.deepEqual([{p:[1],ld:{},li:"brillig"}], type.transform([{p:[0],ld:{},li:"brillig"}], [{p:[0],li:36}], 'left')));

    it('replacement vs. replacement', function() {
      assert.deepEqual([], type.transform([{p:[0],ld:null,li:[]}], [{p:[0],ld:null,li:0}], 'right'));
      return assert.deepEqual([{p:[0],ld:[],li:0}], type.transform([{p:[0],ld:null,li:0}], [{p:[0],ld:null,li:[]}], 'left'));
    });

    it('composes replace with delete of replaced element results in insert', () => assert.deepEqual([{p:[2],ld:[]}], type.compose([{p:[2],ld:[],li:null}], [{p:[2],ld:null}])));
    ...
    it('lm vs lm', function() {
      ...
      return assert.deepEqual([{p:[3],lm:3}], type.transform([{p:[3],lm:2}], [{p:[2],lm:1}], 'left'));
    });
    ...
    it('changes indices correctly around a move', function() {
      ...
      return assert.deepEqual([{p:[1],ld:3,li:4}], type.transform([{p:[0],ld:3,li:4}], [{p:[1],lm:0}], 'left'));
    });

    return it('li vs lm', function() {
      const li = p => [{p:[p],li:[]}];
      const lm = (f,t) => [{p:[f],lm:t}];
      const xf = type.transform;
  ...
  describe('object', function() {
    it('passes sanity checks', function() {
      ...
      return assert.deepEqual({x:'b'}, type.apply({x:'a'}, [{p:['x'], od:'a', oi:'b'}]));
  });
    ...
    it('Ops on deleted elements become noops', function() {
      ...
      return assert.deepEqual([], type.transform([{p:[], t:'text0', o:[{p:9, i:"bite "}]}], [{p:[],od:"agimble s",oi:null}], 'right'));
    });

    it('Ops on replaced elements become noops', function() {
      assert.deepEqual([], type.transform([{p:[1, 0], si:'hi'}], [{p:[1], od:'x', oi:'y'}], 'left'));
      return assert.deepEqual([], type.transform([{p:[1], t:'text0', o:[{p:0, i:'hi'}]}], [{p:[1], od:'x', oi:'y'}], 'left'));
    });
    ...
    it('Deleted data is changed to reflect edits', function() {
      ...
      return assert.deepEqual([], type.transform([{p:["He"],oi:{}}], [{p:[],od:{},oi:"the"}], 'left'));
    });
    ...
    it('If two inserts are simultaneous, the lefts insert will win', function() {
      ...
      return assert.deepEqual([], type.transform([{p:[1], oi:'b'}], [{p:[1], oi:'a'}], 'right'));
    });
    ...
    it('parallel ops on different keys miss each other', function() {
      ...
      return assert.deepEqual([{p:['x'], t:'text0', o:[{p:0, i:"his "}]}], type.transform([{p:['x'],t:'text0', o:[{p:0, i:"his "}]}], [{p:['y'],od:0,oi:1}], 'right'));
    });

    it('replacement vs. deletion', () => assert.deepEqual([{p:[],oi:{}}], type.transform([{p:[],od:[''],oi:{}}], [{p:[],od:['']}], 'right')));
    ...
    it('replacement vs. replacement', function() {
      ...
      const rightHas = type.apply(null, rightOps);
      const leftHas = type.apply(null, leftOps);

      const [left_, right_] = Array.from(transformX(type, leftOps, rightOps));
    ...
    return it('An attempt to re-delete a key becomes a no-op', function() {
      ...
      return assert.deepEqual([], type.transform([{p:['k'], od:'x'}], [{p:['k'], od:'x'}], 'right'));
    });
  });
...
describe('json', () => describe('native type', () => genTests(nativetype)));
