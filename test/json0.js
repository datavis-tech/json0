...
const genTests = function(type) {
  ...
  describe('sanity', function() {
    describe('#create()', () =>
      it('returns null', () => assert.deepEqual(type.create(), null)));
    ...
    describe('#compose()', function() {
      ...
      return it('merges od+oi, od+oi -> od+oi', () =>
        assert.deepEqual(
          [{ p: ['foo'], od: 1, oi: 2 }],
          type.compose(
            [{ p: ['foo'], od: 1, oi: 3 }],
  ...
  describe('string', function() {
    ...
    return describe('#transform()', function() {
      it('splits deletes', () =>
        assert.deepEqual(
          type.transform([{ p: [0], sd: 'ab' }], [{ p: [1], si: 'x' }], 'left'),
          [{ p: [0], sd: 'a' }, { p: [1], sd: 'b' }]
        ));
      ...
      return it('does not throw errors with blank inserts', () =>
        assert.deepEqual(
          type.transform(
            [{ p: ['k', 5], si: '' }],
            [{ p: ['k', 3], si: 'a' }],
  ...
  describe('string subtype', function() {
    describe('#apply()', () =>
      it('works', function() {
        assert.deepEqual(
          'abc',
          type.apply('a', [{ p: [], t: 'text0', o: [{ p: 1, i: 'bc' }] }])
        );
        assert.deepEqual(
          'bc',
          type.apply('abc', [{ p: [], t: 'text0', o: [{ p: 0, d: 'a' }] }])
        );
        return assert.deepEqual(
          { x: 'abc' },
          type.apply({ x: 'a' }, [
            { p: ['x'], t: 'text0', o: [{ p: 1, i: 'bc' }] }
          ])
        );
      }));
  ...
  describe('list', function() {
    describe('apply', function() {
      it('inserts', function() {
        assert.deepEqual(
          ['a', 'b', 'c'],
          type.apply(['b', 'c'], [{ p: [0], li: 'a' }])
        );
        assert.deepEqual(
          ['a', 'b', 'c'],
          type.apply(['a', 'c'], [{ p: [1], li: 'b' }])
        );
        return assert.deepEqual(
          ['a', 'b', 'c'],
          type.apply(['a', 'b'], [{ p: [2], li: 'c' }])
        );
      });
      ...
      it('deletes', function() {
        assert.deepEqual(
          ['b', 'c'],
          type.apply(['a', 'b', 'c'], [{ p: [0], ld: 'a' }])
        );
        assert.deepEqual(
          ['a', 'c'],
          type.apply(['a', 'b', 'c'], [{ p: [1], ld: 'b' }])
        );
        return assert.deepEqual(
          ['a', 'b'],
          type.apply(['a', 'b', 'c'], [{ p: [2], ld: 'c' }])
        );
      });
      ...
      it('replaces', () =>
        assert.deepEqual(
          ['a', 'y', 'b'],
          type.apply(['a', 'x', 'b'], [{ p: [1], ld: 'x', li: 'y' }])
        ));

      return it('moves', function() {
        assert.deepEqual(
          ['a', 'b', 'c'],
          type.apply(['b', 'a', 'c'], [{ p: [1], lm: 0 }])
        );
        return assert.deepEqual(
          ['a', 'b', 'c'],
          type.apply(['b', 'a', 'c'], [{ p: [0], lm: 1 }])
        );
      });
    ...
    describe('#transform()', function() {
      it('bumps paths when list elements are inserted or removed', function() {
        ...
        assert.deepEqual(
          [{ p: [1], ld: 2 }],
          type.transform([{ p: [0], ld: 2 }], [{ p: [0], li: 1 }], 'left')
        );
        return assert.deepEqual(
          [{ p: [1], ld: 2 }],
          type.transform([{ p: [0], ld: 2 }], [{ p: [0], li: 1 }], 'right')
        );
      });
      ...
      it('converts ops on deleted elements to noops', function() {
        ...
        assert.deepEqual(
          [{ p: [0], li: 'x' }],
          type.transform([{ p: [0], li: 'x' }], [{ p: [0], ld: 'y' }], 'left')
        );
        return assert.deepEqual(
          [],
          type.transform([{ p: [0], na: -3 }], [{ p: [0], ld: 48 }], 'left')
        );
      });
      ...
      it('Puts the left op first if two inserts are simultaneous', function() {
        assert.deepEqual(
          [{ p: [1], li: 'a' }],
          type.transform([{ p: [1], li: 'a' }], [{ p: [1], li: 'b' }], 'left')
        );
        return assert.deepEqual(
          [{ p: [2], li: 'b' }],
          type.transform([{ p: [1], li: 'b' }], [{ p: [1], li: 'a' }], 'right')
        );
      });
      ...
      return it('converts an attempt to re-delete a list element into a no-op', function() {
        assert.deepEqual(
          [],
          type.transform([{ p: [1], ld: 'x' }], [{ p: [1], ld: 'x' }], 'left')
        );
        return assert.deepEqual(
          [],
          type.transform([{ p: [1], ld: 'x' }], [{ p: [1], ld: 'x' }], 'right')
        );
      });
    });
    ...
    it('moves ops on a moved element with the element', function() {
      assert.deepEqual(
        [{ p: [10], ld: 'x' }],
        type.transform([{ p: [4], ld: 'x' }], [{ p: [4], lm: 10 }], 'left')
      );
      assert.deepEqual(
        [{ p: [10, 1], si: 'a' }],
        type.transform([{ p: [4, 1], si: 'a' }], [{ p: [4], lm: 10 }], 'left')
      );
      assert.deepEqual(
        [{ p: [10], t: 'text0', o: [{ p: 1, i: 'a' }] }],
        type.transform(
          [{ p: [4], t: 'text0', o: [{ p: 1, i: 'a' }] }],
          [{ p: [4], lm: 10 }],
          'left'
        )
      );
      assert.deepEqual(
        [{ p: [10, 1], li: 'a' }],
        type.transform([{ p: [4, 1], li: 'a' }], [{ p: [4], lm: 10 }], 'left')
      );
      assert.deepEqual(
        [{ p: [10, 1], ld: 'b', li: 'a' }],
        type.transform(
      ...
      assert.deepEqual(
        [{ p: [0], li: null }],
        type.transform([{ p: [0], li: null }], [{ p: [0], lm: 1 }], 'left')
      );
      // [_,_,_,_,5,6,7,_]
      ...
      assert.deepEqual(
        [{ p: [6], li: 'x' }],
        type.transform([{ p: [5], li: 'x' }], [{ p: [5], lm: 1 }], 'left')
      );
      // [_,_,_,_,5,6,7,_]
      ...
      assert.deepEqual(
        [{ p: [1], ld: 6 }],
        type.transform([{ p: [5], ld: 6 }], [{ p: [5], lm: 1 }], 'left')
      );
      //assert.deepEqual [{p:[0],li:{}}], type.transform [{p:[0],li:{}}], [{p:[0],lm:0}], 'right'
      assert.deepEqual(
        [{ p: [0], li: [] }],
        type.transform([{ p: [0], li: [] }], [{ p: [1], lm: 0 }], 'left')
      );
      return assert.deepEqual(
        [{ p: [2], li: 'x' }],
        type.transform([{ p: [2], li: 'x' }], [{ p: [0], lm: 1 }], 'left')
      );
    });
    ...
    it('moves target index on ld/li', function() {
      assert.deepEqual(
        [{ p: [0], lm: 1 }],
        type.transform([{ p: [0], lm: 2 }], [{ p: [1], ld: 'x' }], 'left')
      );
      assert.deepEqual(
        [{ p: [1], lm: 3 }],
        type.transform([{ p: [2], lm: 4 }], [{ p: [1], ld: 'x' }], 'left')
      );
      assert.deepEqual(
        [{ p: [0], lm: 3 }],
        type.transform([{ p: [0], lm: 2 }], [{ p: [1], li: 'x' }], 'left')
      );
      assert.deepEqual(
        [{ p: [3], lm: 5 }],
        type.transform([{ p: [2], lm: 4 }], [{ p: [1], li: 'x' }], 'left')
      );
      return assert.deepEqual(
        [{ p: [1], lm: 1 }],
        type.transform([{ p: [0], lm: 0 }], [{ p: [0], li: 28 }], 'left')
      );
    });
    ...
    it('tiebreaks lm vs. ld/li', function() {
      assert.deepEqual(
        [],
        type.transform([{ p: [0], lm: 2 }], [{ p: [0], ld: 'x' }], 'left')
      );
      assert.deepEqual(
        [],
        type.transform([{ p: [0], lm: 2 }], [{ p: [0], ld: 'x' }], 'right')
      );
      assert.deepEqual(
        [{ p: [1], lm: 3 }],
        type.transform([{ p: [0], lm: 2 }], [{ p: [0], li: 'x' }], 'left')
      );
      return assert.deepEqual(
        [{ p: [1], lm: 3 }],
        type.transform([{ p: [0], lm: 2 }], [{ p: [0], li: 'x' }], 'right')
      );
    });
    ...
    it('lm vs lm', function() {
      assert.deepEqual(
        [{ p: [0], lm: 2 }],
        type.transform([{ p: [0], lm: 2 }], [{ p: [2], lm: 1 }], 'left')
      );
      assert.deepEqual(
        [{ p: [4], lm: 4 }],
        type.transform([{ p: [3], lm: 3 }], [{ p: [5], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [2], lm: 0 }],
        type.transform([{ p: [2], lm: 0 }], [{ p: [1], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [2], lm: 1 }],
        type.transform([{ p: [2], lm: 0 }], [{ p: [1], lm: 0 }], 'right')
      );
      assert.deepEqual(
        [{ p: [3], lm: 1 }],
        type.transform([{ p: [2], lm: 0 }], [{ p: [5], lm: 0 }], 'right')
      );
      assert.deepEqual(
        [{ p: [3], lm: 0 }],
        type.transform([{ p: [2], lm: 0 }], [{ p: [5], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [0], lm: 5 }],
        type.transform([{ p: [2], lm: 5 }], [{ p: [2], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [0], lm: 5 }],
        type.transform([{ p: [2], lm: 5 }], [{ p: [2], lm: 0 }], 'left')
      ...
      assert.deepEqual(
        [{ p: [0], lm: 0 }],
        type.transform([{ p: [1], lm: 0 }], [{ p: [0], lm: 5 }], 'right')
      );
      assert.deepEqual(
        [{ p: [0], lm: 0 }],
        type.transform([{ p: [1], lm: 0 }], [{ p: [0], lm: 1 }], 'right')
      );
      assert.deepEqual(
        [{ p: [1], lm: 1 }],
        type.transform([{ p: [0], lm: 1 }], [{ p: [1], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [1], lm: 2 }],
        type.transform([{ p: [0], lm: 1 }], [{ p: [5], lm: 0 }], 'right')
      );
      assert.deepEqual(
        [{ p: [3], lm: 2 }],
        type.transform([{ p: [2], lm: 1 }], [{ p: [5], lm: 0 }], 'right')
      );
      assert.deepEqual(
        [{ p: [2], lm: 1 }],
        type.transform([{ p: [3], lm: 1 }], [{ p: [1], lm: 3 }], 'left')
      );
      assert.deepEqual(
        [{ p: [2], lm: 3 }],
        type.transform([{ p: [1], lm: 3 }], [{ p: [3], lm: 1 }], 'left')
      );
      assert.deepEqual(
        [{ p: [2], lm: 6 }],
        type.transform([{ p: [2], lm: 6 }], [{ p: [0], lm: 1 }], 'left')
      );
      assert.deepEqual(
        [{ p: [2], lm: 6 }],
        type.transform([{ p: [2], lm: 6 }], [{ p: [0], lm: 1 }], 'right')
      );
      assert.deepEqual(
        [{ p: [2], lm: 6 }],
        type.transform([{ p: [2], lm: 6 }], [{ p: [1], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [2], lm: 6 }],
        type.transform([{ p: [2], lm: 6 }], [{ p: [1], lm: 0 }], 'right')
      );
      assert.deepEqual(
        [{ p: [0], lm: 2 }],
        type.transform([{ p: [0], lm: 1 }], [{ p: [2], lm: 1 }], 'left')
      );
      assert.deepEqual(
        [{ p: [2], lm: 0 }],
        type.transform([{ p: [2], lm: 1 }], [{ p: [0], lm: 1 }], 'right')
      );
      assert.deepEqual(
        [{ p: [1], lm: 1 }],
        type.transform([{ p: [0], lm: 0 }], [{ p: [1], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [0], lm: 0 }],
        type.transform([{ p: [0], lm: 1 }], [{ p: [1], lm: 3 }], 'left')
      );
      assert.deepEqual(
        [{ p: [3], lm: 1 }],
        type.transform([{ p: [2], lm: 1 }], [{ p: [3], lm: 2 }], 'left')
      );
      return assert.deepEqual(
        [{ p: [3], lm: 3 }],
        type.transform([{ p: [3], lm: 2 }], [{ p: [2], lm: 1 }], 'left')
      );
    });
    ...
    it('changes indices correctly around a move', function() {
      assert.deepEqual(
        [{ p: [1, 0], li: {} }],
        type.transform([{ p: [0, 0], li: {} }], [{ p: [1], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [0], lm: 0 }],
        type.transform([{ p: [1], lm: 0 }], [{ p: [0], ld: {} }], 'left')
      );
      assert.deepEqual(
        [{ p: [0], lm: 0 }],
        type.transform([{ p: [0], lm: 1 }], [{ p: [1], ld: {} }], 'left')
      );
      assert.deepEqual(
        [{ p: [5], lm: 0 }],
        type.transform([{ p: [6], lm: 0 }], [{ p: [2], ld: {} }], 'left')
      );
      assert.deepEqual(
        [{ p: [1], lm: 0 }],
        type.transform([{ p: [1], lm: 0 }], [{ p: [2], ld: {} }], 'left')
      );
      assert.deepEqual(
        [{ p: [1], lm: 1 }],
        type.transform([{ p: [2], lm: 1 }], [{ p: [1], ld: 3 }], 'right')
      );

      assert.deepEqual(
        [{ p: [1], ld: {} }],
        type.transform([{ p: [2], ld: {} }], [{ p: [1], lm: 2 }], 'right')
      );
      assert.deepEqual(
        [{ p: [2], ld: {} }],
        type.transform([{ p: [1], ld: {} }], [{ p: [2], lm: 1 }], 'left')
      );

      assert.deepEqual(
        [{ p: [0], ld: {} }],
        type.transform([{ p: [1], ld: {} }], [{ p: [0], lm: 1 }], 'right')
      );

      assert.deepEqual(
        [{ p: [0], ld: 1, li: 2 }],
        type.transform([{ p: [1], ld: 1, li: 2 }], [{ p: [1], lm: 0 }], 'left')
      );
      assert.deepEqual(
        [{ p: [0], ld: 2, li: 3 }],
        type.transform([{ p: [1], ld: 2, li: 3 }], [{ p: [0], lm: 1 }], 'left')
      );
      return assert.deepEqual(
        [{ p: [1], ld: 3, li: 4 }],
        type.transform([{ p: [0], ld: 3, li: 4 }], [{ p: [1], lm: 0 }], 'left')
      );
    });
  ...
  describe('object', function() {
    it('passes sanity checks', function() {
      assert.deepEqual(
        { x: 'a', y: 'b' },
        type.apply({ x: 'a' }, [{ p: ['y'], oi: 'b' }])
      );
      assert.deepEqual({}, type.apply({ x: 'a' }, [{ p: ['x'], od: 'a' }]));
      return assert.deepEqual(
        { x: 'b' },
        type.apply({ x: 'a' }, [{ p: ['x'], od: 'a', oi: 'b' }])
      );
    });
    ...
    it('Ops on deleted elements become noops', function() {
      assert.deepEqual(
        [],
        type.transform([{ p: [1, 0], si: 'hi' }], [{ p: [1], od: 'x' }], 'left')
      );
      assert.deepEqual(
        [],
        type.transform(
    ...
    it('Deleted data is changed to reflect edits', function() {
      assert.deepEqual(
        [{ p: [1], od: 'abc' }],
        type.transform([{ p: [1], od: 'a' }], [{ p: [1, 1], si: 'bc' }], 'left')
      );
      assert.deepEqual(
        [{ p: [1], od: 'abc' }],
        type.transform(
      ...
      assert.deepEqual(
        [{ p: [], od: 25, oi: [] }],
        type.transform([{ p: [], od: 22, oi: [] }], [{ p: [], na: 3 }], 'left')
      );
      assert.deepEqual(
        [{ p: [], od: { toves: '' }, oi: 4 }],
        type.transform(
    ...
    it('If two inserts are simultaneous, the lefts insert will win', function() {
      assert.deepEqual(
        [{ p: [1], oi: 'a', od: 'b' }],
        type.transform([{ p: [1], oi: 'a' }], [{ p: [1], oi: 'b' }], 'left')
      );
      return assert.deepEqual(
        [],
        type.transform([{ p: [1], oi: 'b' }], [{ p: [1], oi: 'a' }], 'right')
      );
    });
    ...
    it('parallel ops on different keys miss each other', function() {
      assert.deepEqual(
        [{ p: ['a'], oi: 'x' }],
        type.transform([{ p: ['a'], oi: 'x' }], [{ p: ['b'], oi: 'z' }], 'left')
      );
      assert.deepEqual(
        [{ p: ['a'], oi: 'x' }],
        type.transform([{ p: ['a'], oi: 'x' }], [{ p: ['b'], od: 'z' }], 'left')
      );
      assert.deepEqual(
        [{ p: ['in', 'he'], oi: {} }],
        type.transform(
    ...
    return it('An attempt to re-delete a key becomes a no-op', function() {
      assert.deepEqual(
        [],
        type.transform([{ p: ['k'], od: 'x' }], [{ p: ['k'], od: 'x' }], 'left')
      );
      return assert.deepEqual(
        [],
        type.transform(
